...
type CreatePackageBookingModalProps = {
    isOpen: boolean;
    closeModal: () => void;
    addNewBooking: (newBooking: PackageBooking) => void;
};

const CreatePackageBookingModal = ({
    isOpen,
    closeModal,
    addNewBooking,
}: CreatePackageBookingModalProps) => {
    const { createBooking, loading } = usePackageBookings();
    const [formData, setFormData] = useState<PackageBooking>({
        fullName: '',
        email: '',
        mobile: '',
        title: '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            alert("Failed to create package booking.");
        } finally {
            setFormData({
                fullName: '',
                email: '',
    ...
    return (
        <div className="inset-0 py-10 flex items-center justify-center bg-gray-200 shadow-2xl">
            <div className="bg-white p-6 rounded-lg shadow-lg w-3/4">
                <h3 className="text-xl font-semibold mb-4">New Package Booking</h3>
...
export default CreatePackageBookingModal;