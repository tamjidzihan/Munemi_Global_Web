...
import { AgentApplication } from "../../../hooks/useAgentApplications";
...
const AgentApplicationList = ({ allApplications, deleteApplication }: AgentApplicationListProps) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [applicationList, setApplicationList] = useState<AgentApplication[]>(allApplications);

    useEffect(() => {
        setApplicationList([...allApplications].sort((a, b) =>
            new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()
        ));
    }, [allApplications]);

    const addNewApplication = (newApplication: AgentApplication) => {
        setApplicationList((prev) => [newApplication, ...prev]);
    };
        ...
        <div className="rounded-sm border border-stroke bg-white shadow-default">
            <div className="py-6 px-4 md:px-6 xl:px-7.5 flex justify-between">
                ...
                >
                    ...
                    {isModalOpen ? `Cancel` : ` Create New Application`}
                </button>
            </div>
            ...
            {applicationList.length > 0 ? (
                applicationList.map((application) => (
                    ...
                    >
                        <div className="col-span-1 flex items-center">
                            <p className="overflow-hidden">{application.tradingName || "N/A"}</p>
                        </div>
                        <div className="col-span-1 flex items-center">
                            <p className="overflow-hidden">{application.businessRegistrationNumber || "N/A"}</p>
                        </div>
                        <div className="col-span-1 flex items-center">
                            <p className="overflow-hidden">{application.companyPhone || "N/A"}</p>
                        </div>

                        <div className="col-span-1 flex items-center">
                            <p className="overflow-hidden text-sm">
                                {application.primaryOfficeLocation || "N/A"}
                                <br />
                                {application.country || "N/A"}
                            </p>
                        </div>
                        <div className="col-span-1 flex items-center">
                            <p className="overflow-hidden">{application.applyingAs || "N/A"}</p>
                        </div>
                        <div className="col-span-2 flex items-center">
                            <p className="text-sm">
                                {application.emailAddress || "N/A"}
                                <br />
                                {application.currentAddress || "N/A"}
                                <br />
                                {application.website || "No website"}
                            </p>
                        </div>
                        <div className="col-span-2 flex items-center">
                            <p className="text-sm">
                ...
                <p className="text-gray-500">No Agent Applications available.</p>
            </div>
            }
        </div>
    );
